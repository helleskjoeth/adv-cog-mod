...
data {
  ...
  real prior_mean_BW;
  real prior_mean_BL;
  real<lower=0> prior_sd_BW;
  real<lower=0> prior_sd_BL;
}
...
model {
  // priors
  target += normal_lpdf(bias_win | prior_mean_BW, prior_sd_BW); // prior of bias. lpdf scales the parameter to a log odds scale
  target += normal_lpdf(bias_lose | prior_mean_BL, prior_sd_BL); // prior of bias
  // model 
  //for (T in 1:t){
...
generated quantities {
  ...
  bias_win_prior = normal_rng(prior_mean_BW, prior_sd_BW); // log-odds scale
  bias_win_posterior = inv_logit(bias_win); 

  bias_lose_prior = normal_rng(prior_mean_BL, prior_sd_BL); // log-odds scale
