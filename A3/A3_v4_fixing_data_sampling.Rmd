...
ggplot(draws_SB, aes(.iteration, SD, group = .chain, color=.chain)) + 
  ...
  theme_classic()
```
It cannot really estimate SD when bias is 0.
It can, however estimate SD when bias does not equal 0.
But when bias = 0,5, the estimate of bias is fixed at 0.5...
...
```
Bias is estimated to be around zero, when the data is simulated with bias = 0.
