...
# make data list
  ...
  trials = trials,
  rating2 = df_SB$rating2, # outcome variable on rating scale 1-8
...
draws_SB %>% mutate(
  ...
  labs(title = "Post. predictions for trial 1",
       x = 'rating') +
  geom_point(x = 6, y = 0, color = "red", shape = 17, size = 3) +
  theme_classic() + 
  labs(x_axis = 'rating2 predictions') +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 2)) +
  

  draws_SB %>% mutate(
  preds = round(inv_logit_scaled(`post_preds[9]`)*9, 0)
  ) %>% 
  ggplot() + 
  geom_histogram(aes(preds)) + 
  labs(title = "Post. predictions for trial 9",
       x = 'rating') + 
  geom_point(x = 4, y = 0, color = "red", shape = 17, size = 3) +
  theme_classic() + 
  scale_x_continuous(breaks = scales::pretty_breaks(n = 2)) +
  
  
    draws_SB %>% mutate(
  preds = round(inv_logit_scaled(`post_preds[17]`)*9, 0)
  ) %>% 
  ggplot() + 
  geom_histogram(aes(preds)) + 
  labs(title = "Post. predictions for trial 17",
       x = 'rating') + 
  geom_point(x = 4, y = 0, color = "red", shape = 17, size = 3) +
  theme_classic() + 
  scale_x_continuous(breaks = scales::pretty_breaks(n = 2)) +
  ...
  draws_SB %>% mutate(
  preds = round(inv_logit_scaled(`post_preds[36]`)*9, 0)
  ) %>% 
  ggplot() + 
  geom_histogram(aes(preds)) + 
  labs(title = "Post. predictions for trial 36",
       x = 'rating') + 
  geom_point(x = 6, y = 0, color = "red", shape = 17, size = 3) +
  theme_classic() + 
  scale_x_continuous(breaks = scales::pretty_breaks(n = 2))
...
data_weighted_Bayes <- list(
  trials = trials,
  rating2 = df_WB$rating2, # outcome variable on rating scale!
...
ggplot(draws_WB, aes(.iteration, w_other, group = .chain, color=.chain)) + 
  geom_line() +
  labs(title = "Weighted Bayes - w_other") +
  theme_classic()
...
# post preds plot
```{r}
# not a lot of variance in the post preds.
  # trial 1, 9, 17, 36
draws_WB %>%  mutate(
  preds = round(inv_logit_scaled(`post_preds[1]`)*9, 0),
  ...
  labs(title = "Post. predictions for trial 1",
       x = 'rating') +
  geom_point(x = 8, y = 0, color = "red", shape = 17, size = 3) +
  theme_classic() + 
  labs(x_axis = 'rating2 predictions') +
...
draws_WB %>%  mutate(
  ...
  labs(title = "Post. predictions for trial 9",
       x = 'rating') +
  geom_point(x = 5, y = 0, color = "red", shape = 17, size = 3) +
  theme_classic() + 
  labs(x_axis = 'rating2 predictions') +
