...
# testing participant 32 and 33 - there is no participant 33
```{r testing par 32}
p32_data <- prep_data(cogsci_data, id = 32)
fit2SB_32 <- fit_model(p32_data, model = "SB", clean = F)


fit2SB_32$cmdstan_diagnose()

# Plot hairy caterpillars
draws_SB <- as_draws_df(fit2SB_32$draws())
...
draws_SB %>% mutate(
  predictions = round(inv_logit_scaled(`post_preds[7]`)*9, 0)) %>% 
  ggplot() + 
  aes(predictions) + 
  ...
  theme_bw()+ 
  labs(title = "Simple Bayes fit to participant 32, faceID 8, trial 7")


# check participant 32, trial 2 - WORSE FIT
draws_SB %>% mutate(
  predictions = round(inv_logit_scaled(`post_preds[8]`)*9, 0)) %>% 
  ggplot() + 
  aes(predictions) + 
  geom_histogram(fill = "#36802d") + 
  ...
  scale_x_continuous(breaks = scales::pretty_breaks(n = 9)) + 
  theme_bw()+ 
  labs(title = "Simple Bayes fit to participant 1, faceID 9, trial 8")
```
...
```
# Model comparison - Sigrid's version
```{r}
