...
transformed parameters {
  ...
  array[ntrials] real rr; // placeholder, ensure bounded values

  for (i in 1: ntrials) {
    
    // calculate distance from obs to all exemplars
    array[(i-1)] real exemplar_sim; // similarity
    for (e in 1: (i-1)){
      array[nfeatures] real tmp_dist; // distance
      for (j in 1: nfeatures) {
        tmp_dist[j] = w[j]*abs(obs[e, j] - obs[i, j]); // distance
      }
      exemplar_sim[e] = exp(-c * sum(tmp_dist)); // similarity
    }
