...
draws_gcm %>% ggplot() +
  geom_histogram(aes(`posteriorpred[10]`), fill = 'lightblue') +
  geom_histogram(aes(`priorpred[10]`), fill = 'pink') + 
  geom_histogram(aes(`posteriorcorrect[10]`), fill = 'green')


draws_gcm %>% ggplot() +
  geom_bar(aes(`priorpred[10]`), fill = 'pink', position = 'stack') +
  ggtitle(label = "Prior") + 
draws_gcm %>% ggplot() +
  geom_bar(aes(`posteriorpred[10]`), fill = 'lightblue', position = 'stack') +
  ggtitle(label = "Posterior") +
draws_gcm %>% ggplot() +
  geom_bar(aes(`posteriorcorrect[10]`), fill = 'green', position = 'stack') + 
  ggtitle(label = "Correct")
```

```{r}
plot_preds <- function(draws, trial_no) {
  prior_col <- paste0("`priorpred[", trial_no, "]`")
  posterior_col <- paste0("`posteriorpred[", trial_no, "]`")
  correct_col <- paste0("`posteriorcorrect[", trial_no, "]`")

  plot <- draws %>% ggplot() +
    geom_bar(aes_string(prior_col), fill = 'pink', position = 'stack') +
    ggtitle(label = "Prior") + 
    draws %>% ggplot() +
    geom_bar(aes_string(posterior_col), fill = 'lightblue', position = 'stack') +
    ggtitle(label = "Posterior") +
    draws %>% ggplot() +
    geom_bar(aes_string(correct_col), fill = 'green', position = 'stack') + 
    ggtitle(label = "Correct")

  return(plot)
}

plot_preds(draws_gcm, 10)
