...
for (i in 1:3) {
  ...
  stim_seq <- c(stim_seq, sequence)
}

# create an (almost) empty tibble with NAs for all the features. Only includes a column with the stimulus sequence
experiment <- tibble(stimulus = stim_seq, spots = NA, eyes = NA, arms = NA, legs = NA, color = NA, category = NA)
...
simulate_responses <- function(agent, w, c){
  ...
  #   weight <-  c(0.1, 0.1, 0.1, 0.1, 0.6)
      ...
      weight <- c(0.35, 0.35, 0.1, 0.1, 0.1)
  }
...
```{r}
#write.csv(simulated_responses, "sim_res.csv")
#write.csv(simulated_responses, "sim_res_w2_a1.csv")
simulated_responses = read_csv('sim_res.csv')
```
...
file <- file.path("stan/gcm_ric.stan")
mod_GCM <- cmdstan_model(file, 
...
d <- simulated_responses %>% subset(
 # c == "1.1" & w == "equal" & agent == '1' # try c == 0.5 ? 
   c == "1.1" & w == "equal" 
...
gcm_data <- list(
  ...
  cat_one = d$category,
  y = d$sim_response,
  obs = as.matrix(d[, c("spots", "eyes", "arms", "legs", "color")]), # features
  b = 0.5,
...
```{r}
# chains
draws_gcm <- as_draws_df(samples_gcm$draws())
#write.csv(draws_gcm, "draws_gcm_w_equal_c_1.1.csv")

# c
ggplot(draws_gcm, aes(.iteration, c, group = .chain, color=.chain)) + 
  geom_line() +
  labs(title = "c") +
  theme_classic() + 
# logit c
ggplot(draws_gcm, aes(.iteration, logit_c, group = .chain, color=.chain)) + 
  geom_line() +
  labs(title = "logit c") +
  theme_classic()

# weights
ggplot(draws_gcm, aes(.iteration, `w[1]`, group = .chain, color=.chain)) + 
  geom_line() +
  labs(title = "w1") +
  theme_classic() +
  ggplot(draws_gcm, aes(.iteration, `w[2]`, group = .chain, color=.chain)) + 
  geom_line() +
  labs(title = "w2") +
  theme_classic() +
  ggplot(draws_gcm, aes(.iteration, `w[3]`, group = .chain, color=.chain)) + 
  geom_line() +
  labs(title = "w3") +
  theme_classic() +
  ggplot(draws_gcm, aes(.iteration, `w[4]`, group = .chain, color=.chain)) + 
  geom_line() +
  labs(title = "w4") +
  theme_classic() +
  ggplot(draws_gcm, aes(.iteration, `w[5]`, group = .chain, color=.chain)) + 
  geom_line() +
  labs(title = "w5") +
  theme_classic() 
...
ggplot(draws_gcm) +
  ...
  geom_vline(xintercept = inv_logit_scaled(d$c[1])) + ## inv logit scaled!
  theme_bw()
```
...
# prior and post estimates of parameters
```{r}
draws_gcm %>% ggplot() +
  geom_density(aes(`r[10]`), fill = 'lightblue') +
  geom_density(aes(`r_prior[10]`), fill = 'pink') + 
  theme_classic() + 
  labs(title = "Model predictions for trial no. 10") + 

draws_gcm %>% ggplot() +
  geom_density(aes(`r[20]`), fill = 'lightblue') +
  geom_density(aes(`r_prior[20]`), fill = 'pink') + 
  theme_classic() + 
  labs(title = "Model predictions for trial no. 20") +

draws_gcm %>% ggplot() +
  geom_density(aes(`r[40]`), fill = 'lightblue') +
  geom_density(aes(`r_prior[40]`), fill = 'pink') + 
  theme_classic() + 
  labs(title = "Model predictions for trial no. 40") +

draws_gcm %>% ggplot() +
  geom_density(aes(`r[70]`), fill = 'lightblue') +
  geom_density(aes(`r_prior[70]`), fill = 'pink') + 
  theme_classic() + 
  labs(title = "Model predictions for trial no. 70")
```
# prior and post preds

```{r plot_preds function}
